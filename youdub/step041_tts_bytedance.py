...
def get_bytedance_config():
    return os.getenv('BYTEDANCE_APPID'), os.getenv('BYTEDANCE_ACCESS_TOKEN')

host = "openspeech.bytedance.com"
api_url = f"https://{host}/api/v1/tts"

def get_request_json(text, voice_type):
...
def tts(text, output_path, speaker_wav, voice_type=None):
    ...
    for retry in range(3):
        try:
            current_request_json = get_request_json(text, voice_type)
            resp = requests.post(api_url, json.dumps(current_request_json), headers=header, timeout=60)
            # print(f"resp body: \n{resp.json()}")
